<div align="center">
  ...
  <p>AI는 기억합니다. 함께 걸어온 여정을 펼쳐 보세요.</p>
...
Claude와의 협업 여정을 함께 펼쳐보는 개인 Journey 화면입니다.
...
**필수:** [Bun](https://bun.sh) 1.1+, **Claude Pro/Max 구독 또는 Anthropic API 키로 로그인된** [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) (Free tier 만으로는 동작하지 않음), [claude-mem](https://github.com/thedotmack) 플러그인 (최소 1회 사용)
...
> **Windows 사용자:** **WSL2 터미널 내부에서 모든 명령어를 실행**하세요.
...
| 필드 | 설명 | 기본값 |
|------|------|--------|
| `userName` | Journey 화면에 표시될 이름 **(필수)** | — |
| `role` | 역할 배지 **(필수)** | `"Developer"` |
| `avatar` | 아바타 이모지 **(필수)** | `"🧑‍💻"` |
| `port` | 서버 포트 | `3000` |
| `language` | Journey 언어 (`en` / `ko`) | `"en"` |
...
| `claude.modelPriority` | 시도할 모델 순서. 운영 실패(rate limit/timeout/unavailable) 시에만 다음 모델로 fallback | `["sonnet", "opus"]` |
...
systemd 활성화 시 Linux와 동일. 미활성화 시 `bun server.js`로 수동 실행.
...
3. **Claude CLI**가 프로필, 관계, 철학, 주간 요약, 오늘의 마음을 동적 생성 (캐시 적용)
4. **Journey View**가 모든 정보를 한 페이지에 시각화

런처가 Bun 런타임을 자동 탐지(mise 또는 system)하므로, Bun 업그레이드 시 재설치가 필요 없습니다.
...
기본적으로 Sonnet을 먼저 시도하고, **명시적인 운영 실패**(rate limit, timeout, provider unavailable)일 때만 Opus로 fallback 합니다. 이 default 는 Claude Pro 의 session/weekly 한도 안에서 안정적으로 동작하도록 설계되어 있습니다. 각 섹션 제목 옆에 사용된 모델이 표시됩니다 (예: `· ✨ sonnet · 5m ago`).
...
Max 사용자거나 API 직접 사용 중이고 처음부터 premium 품질을 원하면 순서를 바꾸세요: `["opus", "sonnet"]`. 한 모델만 강제하려면 한 항목만: `["sonnet"]`.
...
- **Claude Code 구독 (Pro $20, Max 5x $100, Max 20x $200):** 토큰이 플랜에 포함되어 **별도 과금 없음**. ~85K/월은 일반 채팅 1~2개 분량.
- **API 직접 사용:** default sonnet-first 기준 약 $1/월, opus-first 로 바꾸면 약 $2~3/월.
